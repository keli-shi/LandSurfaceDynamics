...
class VisDynamicsModel(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        if self.hparams.model_name == 'encoder-decoder-64':
            self.model = EncoderDecoder64x1x1(in_channels=1)
            # self.model = EncoderDecoder64x1x1(in_channels=3)

        ''' ndvi '''
        if self.hparams.model_name == 'refine-64' and self.hparams.dataset[:4] == 'ndvi':
            self.model = RefineNDVIModel(in_channels=64)

        if 'refine' in self.hparams.model_name and self.hparams.if_test:
            # self.high_dim_model = EncoderDecoder64x1x1(in_channels=3)
            self.high_dim_model = EncoderDecoder64x1x1(in_channels=1)
        # loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.model_name in ['encoder-decoder', 'encoder-decoder-64']:
            self.logger.experiment.add_image('input 0', data[-1, :, :, :128], self.global_step, dataformats='CHW')
            self.logger.experiment.add_image('input 1', data[-1, :, :, 128:], self.global_step, dataformats='CHW')
            self.logger.experiment.add_image('target 0', target[-1, :, :, :128], self.global_step, dataformats='CHW')
            self.logger.experiment.add_image('target 1', target[-1, :, :, 128:], self.global_step, dataformats='CHW')
            self.logger.experiment.add_image('output 0', output[-1, :, :, :128], self.global_step, dataformats='CHW')
            self.logger.experiment.add_image('output 1', output[-1, :, :, 128:], self.global_step, dataformats='CHW')
    ...
    def paths_to_tuple(self, paths):
        ...
        for i in range(len(paths)):
            ...
            tmp = tmp[1].split('\\')
            # print(tmp)
            new_paths.append((int(tmp[1]), int(tmp[2])))
        return new_paths
